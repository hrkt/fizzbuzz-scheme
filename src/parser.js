...
export class FsParser {
  ...
  static tokenize (code) {
    ...
    while (i < code.length) {
      ...
      // vector, boolean letheral, char or number
      if (c === '#') {
        if (i + 1 >= code.length) {
        ...
        if (code.charAt(i + 1) === '\\') {
          ...
          // edge case
          if (i + 'space'.length <= code.length &&
            code.substring(i + 2, i + 2 + 'space'.length) === 'space') {
            buf += '#' + code.charAt(i + 1) + 'space'
            i++ // "#"
            i++ // "\"
            i += 'space'.length // character
          } else {
